...
"""
  ...
  def solve() do
    @input
    |> String.split("\n", trim: true)
    |> Enum.filter(&ipv7_supports_tls?/1)
    |> Enum.count
  end
  ...
  def solve2() do
    @input
    |> String.split("\n", trim: true)
    |> Enum.filter(&ipv7_supports_ssl?/1)
    |> Enum.count
  end

  defp ipv7_supports_tls?(ipv7) do
    {not_in_brackets, in_brackets} = split_ipv7(ipv7)

    if nil != Enum.find(in_brackets, &contains_pair_of_four?/1) do
      false
    else
      nil != Enum.find(not_in_brackets, &contains_pair_of_four?/1)
    end
  end

  defp ipv7_supports_ssl?(ipv7) do
    {not_in_brackets, in_brackets} = split_ipv7(ipv7)

    [] != not_in_brackets
    |> Enum.flat_map(&find_pairs_of_three/1)
    |> Enum.filter(&(&1 != nil))
    |> Enum.filter(fn(aba) ->
  ...
  defp contains_pair_of_four?(<<a, b, b, a, _::binary>>) when a != b, do: true
  defp contains_pair_of_four?(<<_, rest::binary>>), do: contains_pair_of_four?(rest)
  defp contains_pair_of_four?(<<a, b, b, a>>) when a != b, do: true
  defp contains_pair_of_four?(<<_::binary>>), do: false

  defp find_pairs_of_three(string), do: find_pairs_of_three(string, [])
  defp find_pairs_of_three(<<a, b, a, rest::binary>>, acc) when a != b, do: find_pairs_of_three(<<b,a>> <> rest, [<<a,b,a>>|acc])
  defp find_pairs_of_three(<<a, b, a>>, acc)               when a != b, do: find_pairs_of_three(<<b,a>>,         [<<a,b,a>>|acc])
  defp find_pairs_of_three(<<_, rest::binary>>, acc), do: find_pairs_of_three(rest, acc)
  defp find_pairs_of_three(<<_::binary>>, acc), do: acc
